...
export default function file({ accept = "", multiple = false, compact = false } = {}) {
  ...
  for (const [button, icon, key] of [
    ...
    [clear, "close", "chatting.attach.remove"]
  ]) {
    button.type = "button";
  ...
  const update = () => {
    ...
    if (compact) root.hidden = !files.length;
