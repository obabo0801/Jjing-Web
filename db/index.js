...
for (const table of ["web", "fcm"]) {
  const columns = await connection.all(`PRAGMA table_info(${table})`);

  if (!columns.some((column) => column.name === "registered"))
    await connection.exec(`ALTER TABLE ${table} ADD COLUMN registered TEXT`);

  await connection.exec(`
    CREATE TRIGGER IF NOT EXISTS ${table}_registered AFTER INSERT ON ${table}
    WHEN NEW.registered IS NULL BEGIN
      UPDATE ${table} SET registered = datetime('now', '+9 hours') WHERE rowid = NEW.rowid;
    END;
  `);
}
