...
const people = async (records) => {
  ...
  for (const record of records) {
    const user = await db.get(
      `SELECT id, avatar, google IS NOT NULL AS verified,
       CASE WHEN google IS NOT NULL THEN name ELSE '' END AS name
       FROM user WHERE uid = ? AND erased = 0`,
      [record.uid]
    );
...
async function metadata(kind, file) {
  if (kind !== "upload") {
    const record = await db.get(
      `SELECT uid, text, time FROM ${kind} WHERE file = ? ORDER BY time, rowid LIMIT 1`,
      [file.split("/").at(-1)]
    );
  ...
  const recorded = await db.all("SELECT uid, time FROM upload WHERE file = ? ORDER BY time", [
    file
  ]);
  ...
  for (const url of urls) {
    records.push(
      ...(await db.all(
        `SELECT uid FROM profile_file WHERE file = ?
       UNION SELECT uid FROM user WHERE image = ? OR avatar = ?
       UNION SELECT uid FROM chatting WHERE image = ? OR preview = ? OR audio = ?
         OR instr(COALESCE(attachments, ''), ?) > 0
       UNION SELECT sender AS uid FROM message WHERE audio = ?
         OR instr(COALESCE(attachments, ''), ?) > 0`,
...
export async function list(kind, options) {
  ...
  const dates = new Map(
    kind === "upload"
      ? []
      : (await db.all(`SELECT file, MIN(time) AS time FROM ${kind} GROUP BY file`)).map((item) => [
          item.file,
          item.time
        ])
  );
  ...
  if (kind !== "upload" && names.length) {
    const rows = await db.all(
      `SELECT file,text,time FROM ${kind} WHERE file IN (${names.map(() => "?").join(",")}) ORDER BY time,rowid`,
      names
    );
