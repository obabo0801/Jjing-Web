...
async function initialize() {
  ...
  const db = connect(path.data("evidence.db"));
  ...
  await db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS record_subject ON record(subject, expires);
  ...
  for (const item of records) {
    ...
    await db.run(
      `INSERT OR IGNORE INTO record
        (id, subject, kind, reason, time, expires) VALUES (?, ?, ?, ?, ?, ?)`,
...
export const read = async (uid) => {
  ...
  return db.all(
    `SELECT kind, reason, time, expires FROM record
      JOIN member ON member.subject = record.subject
      WHERE member.uid = ? AND expires > ? ORDER BY time DESC LIMIT 100`,
    [uid, Date.now()]
  );
};
...
export const forget = async (uid) => {
  ...
  await db.run("DELETE FROM member WHERE uid = ?", [uid]);
};

export const clean = async () => {
  const db = await open();

  await db.run("DELETE FROM record WHERE expires <= ?", [Date.now()]);
  await db.run(`DELETE FROM member WHERE NOT EXISTS
    (SELECT 1 FROM record WHERE subject = member.subject)`);
};
