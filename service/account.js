        ...
        for (const { name } of tables) {
          ...
          if (collect) {
            ...
            if (name === "notify" && files) {
              const rows = await log.all("SELECT image, body, url FROM notify");

              for (const row of rows)
                for (const value of [row.image, row.body, row.url])
                  for (const file of value?.match(/[a-f0-9]{32}\.[a-z0-9]+/g) || [])
                    files.add(file);
            }
...
const erase = async (uid) => {
  ...
  const chats = await db.all(
    "SELECT id, image, preview, audio, attachments FROM chatting WHERE uid = ?",
    [uid]
  );
  ...
  for (const file of new Set([
    ...
    const shared = await db.get(
      `SELECT 1 FROM user WHERE uid <> ? AND
        ...
        UNION ALL SELECT 1 FROM chatting WHERE uid <> ? AND
          (instr(text, ?) OR instr(COALESCE(attachments, ''), ?)
          OR instr(COALESCE(image, ''), ?) OR instr(COALESCE(preview, ''), ?)
          OR instr(COALESCE(audio, ''), ?))
        UNION ALL SELECT 1 FROM message WHERE
          (instr(text, ?) OR instr(COALESCE(attachments, ''), ?)
          OR instr(COALESCE(audio, ''), ?))
        UNION ALL SELECT 1 FROM upload WHERE uid <> ? AND file = ?
        LIMIT 1`,
      [
        uid,
        name,
  ...
  await db.transaction(async () => {
    await db.run(
      "DELETE FROM chatting_asset WHERE seq IN (SELECT seq FROM chatting WHERE uid = ?)",
      [uid]
    );

    await db.run("DELETE FROM chatting WHERE uid = ?", [uid]);
  });
  ...
  for (const table of ["web", "fcm", "sanction", "block", "authority", "stt", "tts", "upload"])
