...
export const policy = async (uid, peer) => {
  const row = await db.get(
    `SELECT
    EXISTS(SELECT 1 FROM user_block WHERE uid = ? AND peer = ?) AS blocked,
    EXISTS(SELECT 1 FROM user_block WHERE uid = ? AND peer = ?) AS other`,
    [uid, peer, peer, uid]
  );
...
export const find = async (user, id, writable = false) => {
  ...
  const row = await db.get(
    ...
    FROM room r JOIN room_member m ON m.room = r.id
    LEFT JOIN contact c ON c.room = r.id
    JOIN user u ON u.uid = m.uid
    WHERE r.id = ? AND m.uid = ?
      AND (c.room IS NULL OR c.uid = u.uid OR
        (u.role IN (?,?) AND u.erased = 0 AND u.deletion IS NULL))`,
...
const eligible = async (uid, members) => {
  const user = await db.get(
    `SELECT uid,settings FROM user
    WHERE uid = ? AND ${active}`,
    [uid]
  );
...
const record = async (id, type, actor, targets = [], value = "") => {
  ...
  for (const uid of people) {
    const user =
      ...
      (await db.get("SELECT id,name,google FROM user WHERE uid = ?", [uid]));
  ...
  await db.run(
    `INSERT INTO message(id,sender,recipient,room,text,system,time)
    VALUES(?,?,?,?,'',?,?)`,
...
const select = async (user, targets, current = [user]) => {
  ...
  for (const id of ids) {
    const peer = await db.get("SELECT uid FROM user WHERE id = ?", [id]);
...
export async function ensure(user, id) {
  const result = await db.transaction(async () => {
    ...
    const row = await db.get(
      ...
      WHERE r.first = ? AND r.second = ? AND r.multiple = 0
        AND r.closed IS NULL
        AND NOT EXISTS (
          SELECT 1 FROM room_member m
          WHERE m.room = r.id AND m.left IS NOT NULL
        )
    ...
    return db.get("SELECT * FROM room WHERE id = ?", [token]);
  });
...
export function members(id) {
  ...
  FROM room_member m JOIN user u ON u.uid = m.uid
  LEFT JOIN contact c ON c.room = m.room
  WHERE m.room = ? AND (c.room IS NULL OR c.uid = u.uid OR
    (u.role IN (?,?) AND u.erased = 0 AND u.deletion IS NULL))`,
...
export const read = async (user, id) => {
  ...
  const contact = row.contact
    ? await db.get(
        `SELECT u.id,u.name,u.google,c.assigned FROM contact c
        LEFT JOIN user u ON u.uid = c.handler
          AND (c.closed IS NOT NULL OR
            (u.role IN (?,?) AND u.erased = 0 AND u.deletion IS NULL))
        WHERE c.room = ?`,
...
export const search = async (user, query = "", id = "") => {
  ...
  const users = await db.all(
    ...
    FROM user WHERE uid <> ? AND ${active}
    AND (instr(lower(CASE WHEN google IS NOT NULL THEN coalesce(name,'') ELSE '' END), lower(?)) > 0
      OR instr(id, lower(?)) > 0) ORDER BY name,id LIMIT 50`,
    [user.uid, term, term]
  );
  const items = [];
  ...
  for (const candidate of users) {
    ...
    const ended =
      ...
      (await db.get(
        `SELECT 1 FROM room
      WHERE first = ? AND second = ? AND multiple = 0 AND closed IS NOT NULL`,
  ...
  const event = await db.transaction(async () => {
    ...
    for (const peer of peers)
      await db.run(
        ...
        ON CONFLICT(room,uid) DO UPDATE SET left = NULL, reason = NULL,
          deputy = 0`,
        [id, peer.uid]
      );
...
export const manage = async (user, id, action, value) => {
  const event = await db.transaction(async () => {
    ...
    const peer = await db.get(
      ...
      WHERE m.room = ? AND u.id = ? AND m.left IS NULL`,
      [id, value]
    );
...
export const leave = async (user, id) => {
  const result = await db.transaction(async () => {
    ...
    if (row.contact) {
      ...
      await db.run(
        "UPDATE contact SET closed = coalesce(closed,datetime('now','+9 hours')) WHERE room = ?",
        [id]
      );
    ...
    if (row.multiple && !row.contact && !others.length) {
      await db.run(
        `UPDATE room SET closed = datetime('now','+9 hours')
        WHERE id = ?`,
        [id]
      );
    }
    ...
    await db.run(
      `UPDATE room_member
      SET left = datetime('now','+9 hours'), reason = 'leave',
        deputy = 0
      WHERE room = ? AND uid = ?`,
      [id, user.uid]
    );
