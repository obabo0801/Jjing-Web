...
const silence = (stream, signal, stop) =>
  new Promise((resolve) => {
    ...
    if (!audio) {
      ...
      off = dom.on(signal, "abort", () => done(false), {
        once: true
      });
      stopOff = dom.on(stop, "abort", () => done(false), {
        once: true
      });
    ...
    off = dom.on(signal, "abort", done, { once: true });
    stopOff = dom.on(stop, "abort", done, { once: true });
