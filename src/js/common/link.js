...
i18n.preload(...keys.map((key) => `link.${key}`));
...
async function confirm(url, options, run) {
  ...
  root.append(
    ...
    origin,
    destination,
    node("p", "link-notice", scope === "content" ? "embed" : external ? "notice" : "local"),
    field,
    error
  );

  return dialog({
    title: options.file ? "link.file" : scope === "content" ? "link.embed" : "link.title",
    content: root,
    direction: "→",
    actions: [
...
export function open(value, options = {}) {
  ...
  if (local && target === "_self" && !options.file) {
    ...
    if (["terms", "privacy"].includes(name) && !url.search) {
      return import("./legal.js").then(({ default: legal }) => legal(name));
    }
  }
