...
const keys = [
  ...
  ...["master", "system", "media", "notify", "tts"].map((key) => `sound.${key}`)
];
...
const slider = (key, value, min, save, icon) => {
  ...
  const label = node("span", "group-name", key);
  const line = node("div", "range-line");
...
const audio = () => {
  ...
  const items = Object.entries(icons).map(([key, icon]) =>
    slider(
      `sound.${key}`,
      storage.get(`volume-${key}`, key === "system" ? 0 : 100),
      0,
