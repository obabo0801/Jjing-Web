...
export async function change(message, options, selected, quick = false) {
  ...
  try {
    const result = await dialog({
      title: restoring ? "restore.title" : "chatting.removeTitle",
      content: preview(message, selected?.content, selected),
      direction: "→",
      ready: quick
        ? (element) => {
            element.tabIndex = -1;
      ...
      actions: [
        ...
        items.spec(restoring ? "restore" : "remove", { value: true })
      ]
    });
...
export async function menu(message, options = {}, selected, target) {
  ...
  const shared = !(options.private && options.kind === "message");

  if (user && shared) render(user);
  const off = user?.id && shared ? profile.bind(element, user.id, render) : () => {};
