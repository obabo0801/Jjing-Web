...
export default function select(field, attach) {
  ...
  const catalogLoading = progress({ type: "circular", value: 25, show: false });
  const groups = [];
  const basic = [
  ...
  const display = (items, append = false) => {
    ...
    for (const item of items) {
      ...
      if (remote) {
        ...
        busy.element.hidden = true;
        button.append(busy.element);
        dom.on(image, "error", () => {
          image.hidden = true;
          busy.element.hidden = false;
  ...
  async function load() {
    ...
    try {
      ...
      if (!signal.aborted) {
        fetching = false;
        loading.element.hidden = !current.error;
        schedule();
      }
    }
  }
