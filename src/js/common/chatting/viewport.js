...
export default function viewport(root) {
  ...
  const zoomed = () => view && Math.abs((view.scale || 1) - 1) > 0.01;
  ...
  const fit = () => {
    ...
    frame = requestAnimationFrame(() => {
      ...
      css.set(root, { "--chatting-width": `${Math.floor(view?.width || width)}px` });
      ...
      const next = Math.max(0, Math.floor(visible - top - reserved));
  ...
  return () => {
    ...
    if (app) {
      css.set(app, { "--chatting-page-height": null });
    }
  };
}
